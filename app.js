...
const detectIntent = async (languageCode, queryText, sessionId) => {
    ...
    const intents = Object.entries(obj).map(([key, value]) => createIntent(key, value.stringValue));

    return {response: result.intent.displayName, entities: intents}
};

async function interact(userID, request) { // call the Voiceflow API with the user's name & request, get back a response
